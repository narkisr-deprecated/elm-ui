...
update noErrors msg ({ next, prev, default, zero, step } as model) =
    case msg of
        Next ->
            let
                nextStep =
                    withDefault default (List.head next)

                nextSteps =
                    defaultEmpty (List.tail next)

                prevSteps =
                    if step /= zero then
                        List.append prev [ step ]
                    else
                        prev
            in
                if noErrors then
        ...
        Back ->
            let
                prevStep =
                    withDefault zero (List.head (List.reverse prev))

                prevSteps =
                    List.take ((List.length prev) - 1) prev

                nextSteps =
                    if step /= zero then
                        List.append [ step ] next
                    else
                        next
            in
                if noErrors then
