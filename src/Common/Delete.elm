...
deleteView { name } type_ cancel delete =
    dangerCallout (deleteMessage name type_) (div [] []) cancel delete


view ({ errorMsg } as model) type_ cancel delete done =
    if errorMsg /= "" then
        dangerCallout (error errorMsg) (div [] []) cancel done
    else
        deleteView model type_ cancel delete
