...
setSession ({ side, header } as model) session =
    let
        ( newSide, _ ) =
            Side.update (Side.SetSession session) side

        ( newHeader, _ ) =
            Header.update (Header.SetSession session) header
    in
...
sideView { side } =
    Html.map SideMsg (Side.view side)
