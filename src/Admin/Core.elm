...
setOwners model owners =
    let
        users =
            List.map .username owners

        user =
            Maybe.withDefault "" (List.head users)
    in
...
setEnvironments model es =
    let
        environment =
            (Maybe.withDefault "" (List.head (Dict.keys es)))
    in
...
view ({ environments, environment, owner } as model) =
    div []
        [ group_ "Environment" (selector SelectEnvironment environments environment)
        , group_ "Owner" (selector SelectOwner (ownersList model) owner)
        ]
