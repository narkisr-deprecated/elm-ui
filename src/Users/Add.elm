...
merge ({ value, form } as step) acc =
    let
        user =
            withDefault acc (Form.getOutput form)
    in
        case value of
            Main ->
                user
...
setRoles ({ wizard } as model) roles =
    let
        role =
            (Maybe.withDefault "" (List.head (Dict.values roles)))

        mainStep =
            (step (Main.init role) Main)

        pairs =
            List.map (\( f, s ) -> ( s, f )) (Dict.toList roles)
    in
...
update msg ({ wizard } as model) =
    case msg of
        ...
        WizardMsg wizardMsg ->
            let
                newWizard =
                    Wizard.update wizardMsg wizard
            in
        ...
        FormMsg formMsg ->
            let
                newWizard =
                    Wizard.update (Wizard.FormMsg formMsg) wizard
            in
                none { model | wizard = newWizard }
        ...
        Just ({ value } as current) ->
            case value of
                Main ->
                    dialogPanel "info"
                        (info "Add a new User")
                        (panel (fixedPanel (Html.map FormMsg (Main.view roles current))))

                Perm ->
                    dialogPanel "info"
                        (info "User permissions")
                        (panel (fixedPanel (Html.map FormMsg (Perm.view environments operations current))))

        Nothing ->
            dialogPanel "info"
                (info "Save new user")
                (panel (fixedPanel (Html.map (\_ -> NoOp) (summarize (merged model)))))


errorsView { saveErrors } =
    let
        body =
            (Html.map ErrorsView (Errors.view saveErrors))
    in
